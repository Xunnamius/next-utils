...
/**
 * Source: https://github.com/vercel/next.js/blob/f6998e30a815e976173501a62e6e481f92699d81/packages/next/src/build/babel/plugins/next-ssg-transform.ts
 */
...
export default function nextTransformSsg({
  ...
  function getIdentifier(
    ...
    const { parentPath, node: pathNode } = path;
    ...
    if (pathNode.type === 'ArrowFunctionExpression') {
      return null;
    }

    return pathNode.id && pathNode.id.type === 'Identifier'
    ...
    if (b?.referenced) {
      ...
      if (b.path.type === 'FunctionDeclaration') {
        return !b.constantViolations
          ...
          .every((path) => path.findParent((p) => p === b.path));
      }
        ...
        enter(path, state) {
          ...
          path.traverse(
            {
              VariableDeclarator(variablePath, variableState) {
                switch (variablePath.node.id.type) {
                  case 'Identifier': {
          ...
          function sweepImport(
            ...
            if (refs.has(local) && !isIdentifierReferenced(local)) {
              ...
              sweepPath.remove();
              if (
                (sweepPath.parent as BabelTypes.ImportDeclaration).specifiers.length === 0
              ) {
                sweepPath.parentPath.remove();
              }
            }
